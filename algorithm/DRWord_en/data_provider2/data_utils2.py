...
def label_to_array(label):
    try:
        label = label.replace(' ', '')
        return [config_en.CHAR_VECTOR.index(x) for x in label]
    except Exception as ex:
        print(label)
        raise ex
        #pass

def ground_truth_to_word2(ground_truth):
    """
        Return the word string based on the input ground_truth
    """

    try:
        return ''.join([config_en.CHAR_VECTOR[i] for i in ground_truth if i != -1])
    except Exception as ex:
        print(ground_truth)
        print(ex)
        input()
...
def sort_rectangle(poly):
    ...
    if np.count_nonzero(poly[:, 1] == poly[p_lowest, 1]) == 2:
        # 底边平行于X�? 那么p0为左上角 - if the bottom line is parallel to x-axis, then p0 must be the upper-left corner
