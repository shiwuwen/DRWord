...
tf.app.flags.DEFINE_integer('batch_size_per_gpu', 6, '')
tf.app.flags.DEFINE_integer('num_readers', 4, '')
tf.app.flags.DEFINE_float('learning_rate', 0.00001, '')
tf.app.flags.DEFINE_integer('max_steps', 300001, '')
tf.app.flags.DEFINE_float('moving_average_decay', 0.997, '')
tf.app.flags.DEFINE_string('gpu_list', '0,1', '')
tf.app.flags.DEFINE_string('checkpoint_path', 'checkpointmtwi/', '')
...
#/home/wsw/deeplearning/AdEAST/checkpointmtwi/
...
tf.app.flags.DEFINE_string('training_data_dir', default='/home/wsw/temp/MTWI/image_train/', help='training images dir')
#/home/wsw/temp/RCTW/rctw_train/ /home/wsw/temp/ICDAR2015/ch4_training_images/ ch4_test_images/ /home/wsw/temp/ICDAR2017/ch8_training_images/  /home/wsw/temp/MTWI/image_train/
tf.app.flags.DEFINE_string('training_gt_data_dir', default='/home/wsw/temp/MTWI/txt_train/', help='training gt dir')
#/home/wsw/temp/ICDAR2015/ch4_training_localization_transcription_gt/ /home/wsw/temp/ICDAR2017/ch8_training_localization_transcription_gt_v2/ /home/wsw/temp/ICDAR2015/Challenge4_Test_Task1_GT/ /home/wsw/temp/MTWI/txt_train/
...
def build_graph(input_images, input_transform_matrix, input_box_masks, input_box_widths, input_seq_len):
    ...
    return f_geometry, recognition_logits

def compute_loss(f_geometry, recognition_logits, input_geo_maps, input_training_masks, input_transcription, input_box_widths, lamda=0.01):
    
    detection_loss = detect_part.quad_loss(input_geo_maps, f_geometry, input_training_masks)
...
def main(argv=None):
    ...
    input_geo_maps = tf.placeholder(tf.float32, shape=[None, None, None, 7], name='input_geo_maps')
    ...
    f_geometry, recognition_logits = build_graph(input_images, input_transform_matrix, input_box_masks, input_box_widths, input_seq_len)
    ...
    d_loss, r_loss, model_loss = compute_loss(f_geometry, recognition_logits, input_geo_maps, input_training_masks, input_transcription, input_box_widths)
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(FLAGS.max_steps):
            #print('start step : {:6}'.format(step))
            data = next(dg)
            #print('shiyongde shujushi :' + data[10])
            
            inp_dict = {input_images: data[0],
                        #input_score_maps: data[2],
                        input_geo_maps: data[2],
                        input_training_masks: data[3],
                        input_transform_matrix: data[4],
                        input_box_widths: data[6],
                        input_transcription: data[7]}

            for i in range(FLAGS.batch_size_per_gpu):
                inp_dict[input_box_masks[i]] = data[5][i]
