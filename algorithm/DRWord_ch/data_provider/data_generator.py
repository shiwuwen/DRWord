...
from data_provider import data_advance
    ...
    while True:
        ...
        for i in index:
            try:
                ...
                #file_name = im_fn.replace(im_fn.split('.')[-1], 'txt').split('/')[-1] # using for synthtext
                file_name = im_fn[:-4] + '.txt'
                ...
                new_h, new_w, _ = im.shape
                resize_h = input_size #512
                resize_w = input_size #512
                ...
                _, _, training_mask, rectangles = generate_rbox((new_h, new_w), text_polys, text_tags)

                #print('text_ploys before change : ', text_polys[0, :])
                text_polys = data_advance.batch_reorder_vertexes(text_polys)
                #print('text_ploys after change : ', text_polys[0, :])
                
                geo_map = data_advance.generate_box((new_h, new_w), text_polys, text_tags, im_fn, im)
                #print('geo_map : ' ,geo_map[40:50,40:50,0:7])
                ...
                im = preprocess_input(im, mode='tf')
                ...
                if len(batch_images) == batch_size:
                    ...
                    yield batch_images, batch_image_fns, batch_geo_maps, batch_training_masks, batch_transform_matrixes, batch_boxes_masks, batch_box_widths, batch_text_labels_sparse, batch_text_polyses, batch_text_labels
                    batch_images = []
                    batch_image_fns = []
                    #batch_score_maps = []
                    batch_geo_maps = []
